...
**모든 데이터(여행 목록·일정·상세정보·메모·레시피·기타)가 Firebase(Firestore)에서 읽어와 표시**되며, 관리자는 화면에서 직접 만들고 편집합니다. **열람은 로그인 없이 누구나 가능**하고, 로그인하면 관리자는 편집을, 일반 사용자는 의견(댓글) 작성을 할 수 있습니다. 여행/레시피는 각각 **공유 링크 복사** 버튼으로 주소를 바로 복사할 수 있습니다.
...
- **일반 사용자**는 로그인해도 편집은 불가하지만, 여행/레시피 하단에 **의견(댓글)**을 남길 수 있고 본인 댓글은 수정/삭제할 수 있습니다.
...
여행 상세 페이지 하단과 각 레시피/기타 문서 하단에 **의견** 영역이 있습니다.

- 로그인한 사용자(관리자 포함)라면 누구나 의견을 남길 수 있습니다. 로그인 전에는 "로그인하고 의견 남기기" 버튼만 보입니다.
- 본인이 쓴 의견은 연필(수정)/휴지통(삭제) 아이콘으로 직접 고치거나 지울 수 있고, 관리자는 아무 의견이나 삭제할 수 있습니다.
- 의견은 Firestore `comments` 컬렉션에 저장되며, 여행은 `trip:여행id`, 레시피/기타는 `recipe:문서id`·`misc:문서id`를 대상(target)으로 구분합니다.
...
3. Firestore 화면의 **규칙(Rules)** 탭을 열고 아래 규칙으로 교체 후 **게시(Publish)**:
   ...
   (**읽기는 로그인 없이 누구나 허용**, 쓰기는 관리자 이메일 2개만 허용. `trips`는 여행 목록/메타, `notes`는 일정 카드 메모, `itineraries`는 일정 카드, `tripinfo`는 상세 정보 탭 데이터, `recipes`는 레시피, `memos`는 기타 메모입니다. `comments`(의견)는 **로그인한 사람이면 누구나 작성**할 수 있고, **본인 글만 수정**, 삭제는 본인 또는 관리자만 가능합니다.)
...
8. 커밋 후 `main`에 푸시하면 재배포됩니다. 사이트는 로그인 없이 바로 열람되고, 우측 상단 로그인 아이콘으로 구글 로그인을 하면 됩니다. `setario87@gmail.com` 또는 `hd3311@gmail.com`으로 로그인하면 편집까지 가능하고, 그 외 계정은 열람 + 의견 작성이 가능합니다.
