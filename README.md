...
- CSV 열: `day, order, presetKey, time, title, desc, mapQuery` (최소 `day`, `title`만 있으면 됩니다).
- **한계**: CSV 가져오기는 **현재 열려 있는 여행의 일정만** 업데이트합니다. 완전히 새로운 여행(새 페이지)을 CSV만으로 만들 수는 없습니다 — 정적 사이트라 새 HTML 파일 생성은 저장소에 직접 파일을 추가해야 하기 때문입니다 (위 "새 여행 추가하는 법" 참고). 상세 정보 탭 내용은 CSV에 포함되지 않습니다 (상세 정보는 화면에서 직접 편집).

## 하루 동선 지도

각 일차에 장소(구글지도 검색어)가 2곳 이상 있으면, 그 일차 상단에 **"하루 동선 지도로 보기"** 버튼이 나타납니다. 누르면 그날의 장소들을 방문 순서대로 연결한 구글지도 길찾기(경로)가 새 탭에서 열립니다.

- 순서는 일정 카드 순서(드래그로 조정한 순서)를 그대로 따릅니다.
- 장소 링크가 **URL로 되어 있는 카드는 경로에 포함되지 않습니다** (좌표를 알 수 없어서). 동선에 넣고 싶은 장소는 장소명(구글지도 검색어)으로 입력하세요.
- 구글지도 경로의 경유지는 최대 약 9곳까지 지원됩니다.
