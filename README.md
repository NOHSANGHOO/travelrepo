...
### 브라우저 캐시 (배포가 반영되지 않을 때)

정적 사이트라 브라우저가 `assets/*.js`를 캐시해 두면, HTML은 새로 받아도 **자바스크립트는 예전 것이 실행되어** 방금 배포한 변경이 일부만 반영된 것처럼 보일 수 있습니다.

이를 막기 위해 배포 워크플로우가 업로드 직전에 HTML 안의 로컬 asset 주소에 **커밋 해시를 쿼리로 붙입니다** (`assets/docs.js?v=1a2b3c4d`). 배포마다 주소가 달라지므로 브라우저가 항상 새 파일을 받아갑니다. 저장소의 원본 HTML은 그대로이고, 배포 산출물에만 적용됩니다.
...
레시피는 목록 **상단의 수평 탭바**(한식 · 양식 · 중식 · 기타)로 카테고리를 전환합니다 (기타 메모에는 카테고리가 없습니다).

- 각 탭에는 레시피 개수가 함께 표시되고, 선택된 탭은 카테고리 고유 색으로 채워집니다. 탭을 바꾸면 목록이 살짝 올라오며 나타납니다. 기본 선택은 **한식**입니다.
- **보던 카테고리가 유지됩니다**: 레시피를 열었다가 뒤로 나오면 직전에 보던 탭이 그대로 선택되어 있습니다. 선택 탭은 URL(`&cat=`)과 세션에 함께 기록되어, 브라우저 뒤로가기로 돌아와도 복원됩니다.
- **검색 중에는 카테고리를 가리지 않고 전체에서** 찾아 보여주며, 각 결과 카드에 어느 카테고리인지 칩으로 표시합니다.
- 관리자는 카드 왼쪽 손잡이(⠿)를 잡고 **상단 탭 위로 드래그해서 레시피의 카테고리를 옮길 수 있습니다** (마우스·터치 모두 지원). 끌고 가는 동안 대상 탭이 점선으로 강조되고, 놓으면 즉시 Firestore(`category` 필드)에 저장되며 "○○(으)로 옮겼어요" 토스트가 뜹니다.
- 새 레시피를 만들거나 편집할 때 카테고리를 드롭다운으로 고를 수 있습니다. 카테고리가 지정되지 않은 기존 레시피는 "기타"로 표시됩니다.
- 편집 화면의 **저장/취소 버튼은 화면 하단에 고정**되어, 레시피가 길어져도 아래로 스크롤하지 않고 바로 누를 수 있습니다.
