...
function renderTrips(filterKey) {
    ...
    filtered.forEach(({ trip, status }) => {
        ...
        card.className = viewable
            ? "block bg-white rounded-2xl shadow-sm shadow-stone-200/40 border border-stone-100 overflow-hidden hover:shadow-lg hover:shadow-stone-200/60 hover:-translate-y-0.5 transition-all"
            : "block bg-white rounded-2xl shadow-sm shadow-stone-200/40 border border-stone-100 overflow-hidden opacity-70 cursor-not-allowed";
